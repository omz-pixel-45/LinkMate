...
const getLinkIcon = (url, type) => {
    // Prioritize selected type if it's not 'Other'
    if (type && type !== 'Other') {
...
function LinkInputForm() {
    // State to hold the array of link objects
    const [links, setLinks] = useState([]);
    // State for modal visibility and content
    const [showModal, setShowModal] = useState(false);
    const [modalMessage, setModalMessage] = useState('');
    const [isSuccess, setIsSuccess] = useState(false);
    // State for submit button pending status
    const [isSubmitting, setIsSubmitting] = useState(false);

    // Effect to read 'n' from URL on component mount
    useEffect(() => {
        const queryParams = new URLSearchParams(window.location.search);
        const n = parseInt(queryParams.get('n') || '1', 10); // Default to 1 if 'n' is not found or invalid
        

        // Initialize links array with 'n' empty link objects
        const initialLinks = Array.from({ length: Math.max(1, n) }, () => ({
            url: '',
            type: 'Other',
            name: '', // ✅ new field
    ...
    const handleAddLink = () => {
        setLinks(prevLinks => [
            ...prevLinks,
            { url: '', type: 'Other', name: '', id: crypto.randomUUID() }
        ]);
    };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const dataToSend = links.filter(link => link.url.trim() !== '');
            ...
            const response = await axios.post( ServerLink("/links"), { links: dataToSend, uId: userId }, { withCredentials: true });
                    ...
                    {links.map((link, index) => (
                        <div key={link.id} className="link-field">
                            ...
                            <input
                                ...
                                placeholder="Link here (e.g., https://example.com/profile)"
                                value={link.url}
